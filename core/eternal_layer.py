...
class EternalLayer:
    ...
    def _init_git(self):
        ...
        if not git_dir.exists():
            ...
            subprocess.run(["git", "config", "user.email", "bio@memory.local"], cwd=self.base_path, capture_output=True)
            subprocess.run(["git", "config", "user.name", "BioMemory"], cwd=self.base_path, capture_output=True)
...
"""
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", f"{commit_msg}"], cwd=self.base_path, capture_output=True)
